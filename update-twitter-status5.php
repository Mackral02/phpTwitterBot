<?php
	...
	if($quotesArray){
		...
		// echo "<br/>".$fullTweet;
		// echo "<br/>Tweet character count is".strlen($fullTweet);
		...
		if(strlen($fullTweet)> $twitterCharacterLimit){
			...
			// echo "<br/>".count($failedArray)." quotes failed";
			$responseJSON['failedJsonCount'] = count($failedArray);
		...
		if(!empty($tweetQuote)){
			...
			$responseJSON['error'] = 'no quote received';
		}
	}else{
		// echo "<br/>json/array is empty";
